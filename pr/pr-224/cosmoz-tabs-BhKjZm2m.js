...
const useNotifyProperty = (name, value, deps = [value]) => {
    const host = useHost();
    useEffect(() => {
...
const hashUrl = () => new URL(location.hash.replace(/^#!?/iu, '').replace('%23', '#'), location.origin), parameterize = (hashParam) => hashParam
    ? () => new URLSearchParams(hashUrl().hash.replace('#', '')).get(hashParam)
    : undefined;
const link = (hashParam, value) => {
    if (!hashParam) {
        return;
    }
    const url = hashUrl(), sp = new URLSearchParams(url.hash.replace('#', ''));
    if (value == null) {
        sp.delete(hashParam);
    }
    else {
...
const useHashParam = (hashParam) => {
    const parameterized = useMemo(() => parameterize(hashParam), [hashParam]), [param, setParam] = useState(parameterized), ref = useRef(param);
...
const isValid = (tab) => !tab.hidden && !tab.disabled, getIcon = (tab, isSelected) => isSelected ? tab.selectedIcon ?? tab.icon : tab.icon, getName = (tab) => tab.getAttribute("name"), getIconStyle = (tab) => {
  const iconColor = tab.iconColor ?? "#15b0d3";
  return ["color: " + iconColor, tab.iconStyle].join(";");
}, valid = (tabs) => tabs.find(isValid), choose = (tabs, selected) => {
  if (selected == null) {
    return valid(tabs);
  }
...
const toggleSize = (prop) => (el, visible, opts) => {
    ...
    anim.onfinish = () => Object.assign(el.style, {
        [maxSizeProp]: '',
...
const CosmozTabCard = (host) => {
  const { heading, collapsable, collapsed: isCollapsed } = host, headingAttribute = useMemo(
    () => host.getAttribute("heading"),
    [host.heading]
  ), hasSlot = host.querySelector("[slot]"), [collapsed, setCollapsed] = useState(Boolean(isCollapsed)), toggleCollapsed = () => {
